 ...
 *
 */
#define __KS885X_GLB__H__

//The hardware board is ks8695x
#define  UBOOT   1
//All definitions in this section are all hardware board design related
//select one of following KS885X target flags or get from configuration process
#define CONFIG_KS8851_MLL
#define CONFIG_KS885X_BUS_16BIT
//KS8851 MLL CMD address define here or get from configuration process
#define CONFIG_KS8851_MLL_CMD          0x00000002
#define CONFIG_KS8851_BASE_ADDR        0x30000000

#include <common.h>
#include <asm/arch/gpio.h>
#define IO_ADDRESS(x)                  (x)
#define MALLOC(x)                                          malloc(x)
#define FREE_MALLOC(x)                         free(x)
#define UDELAY(x)                                          udelay(x)
typedef unsigned long long int ULONGLONG;

/* Support for KS885X debug info output */
/* #define KS885X_DEBUG */
...
#define  KS885X_CHIP_TYPE

#define CONFIG_KS885X_BUS_16BIT

// Board interrupt for KS8851
//#define CONFIG_KS8851_IRQ                                     33 //dummy number to void compiler error
#define CONFIG_KS8851_IRQ                                      AT91_PIN_PD21
#define CMD_HIGH        CONFIG_KS8851_MLL_CMD  /* data line is address bus */
#define CMD_LOW         0      /* data line is data bus */
//The BE0, BE1, BE2 and BE3 only valid at CONFIG_KS885X_BUS_16BIT mode
// In the CONFIG_KS885X_BUS_8BIT mode, all BEx pin should be tied to GRAND.
...
// KS885X MQL chip we assume pin BE0, BE1, BE2 and BE3 has been set to following condition

#define DATA_ALIGNMENT                         2       // WORD boundary
...
#define KS885X_REG_READ                HW_READ_WORD
#define KS885X_REG_WRITE       HW_WRITE_WORD
...
//all ks885x chip data aligment is 4 byte
...
typedef enum {
       POLLING_ONE_BUFFER = 0,
       POLLING_MULTI_BUFFER,
       INTERRUPT_RECV,
} RECVFLAG;
...
/* Transmit multiplex frame length info */
typedef struct {
       unsigned char *txData;  /* tx data pointer */
       unsigned short txLength;        /* Byte count */
} TX_FRAME_INFO;

/* Receive multiplex framer header info */
typedef struct {
       unsigned short rxStatus;        /* Frame status */
       unsigned short rxLength;        /* Byte count */
} FR_HEADER_INFO;

typedef struct {
       void *m_pUserData;      // store and carry user data in call back function
       unsigned long m_ulIoPhyAddr;
       unsigned long m_ulIoVirtualAddr;
       unsigned short m_wInterruptMask;
       unsigned long m_dwPrevData;
       unsigned short m_uPrevByte;
       FR_HEADER_INFO *m_pRevHdrInfo;  //receive header info array pointer
       unsigned long m_uCurFrameIndex;
       unsigned long m_rxFrameCount;
       unsigned long m_uCurRecvFrams;
       unsigned char m_recvBuffer[RX_BUF_SIZE];
       BOOL m_bEnabled;
       unsigned char m_MacAddress[6];
       struct eth_device netdev;
} KS885XHW, *PKS885XHW;

typedef int (*DRIVER_PREINIT) (int n);

#define PRINT_DATA(pData,length)\
{\
       int i;\
...
#define __KS885X_BOARD_H__
///////////////// following code come from ks8851Reg.h //////////////////////////
...
///////////////// following code come from ks885x_target.h //////////////////////////
#define __KS885X_TAREGT__H__

#define PHY_RESET_TIMEOUT  10
#define CHIP_ID_8851           0x8870
#define KS885X_MULTICAST_SIZE       3

void HW_WRITE_16BITDATA(void *pks885xHW, unsigned short data);
void HW_WRITE_32BITDATA(void *pks885xHW, unsigned long data);
void HW_READ_16BITDATA(void *pks885xHW, unsigned short *data);
void HW_READ_32BITDATA(void *pks885xHW, unsigned long *data);
...
void HW_READ_WORD(void *pks885xHW, unsigned short addr, unsigned short *pw);
void HW_WRITE_WORD(void *pks885xHW, unsigned short addr, unsigned short w);
void HW_WRITE_DATA_HEADER(void *pks885xHW, unsigned short uOrgPktLen);
...
#define HW_READ_START( pks885xHW )                            \
{                                                              \
    KS885X_REG_WRITE( pks885xHW,REG_RX_ADDR_PTR, ADDR_PTR_AUTO_INC );       \
    KS885X_REG_WRITE( pks885xHW, REG_RXQ_CMD, (RXQ_CMD_CNTL | RXQ_START));   \
}

#define HW_READ_END(pks885xHW)                                      \
{                                                              \
    KS885X_REG_WRITE( pks885xHW, REG_RXQ_CMD, RXQ_CMD_CNTL );          \
}

#define HW_WRITE_START(pks885xHW)                                       \
{                                                              \
    unsigned short w;\
...
///////////////// following code come from target_mll.h //////////////////////////
#define __TARGET_MTL_H
